...
class ChartingState extends MusicBeatState
{
	...
	var camYoffset:Int = 240;		//谱面视野整体往上移一点
	//考虑到改动可能会引起的不适，按F10恢复到原pe的谱面视野
	...
	var preRenderedSustains:FlxTypedGroup<FlxSprite>;
	var preRenderedNotes:FlxTypedGroup<Note>;

	var gridBG:FlxSprite;
	var nextGridBG:FlxSprite;
	var preGridBG:FlxSprite;
	...
	**/
	...
	var ifNotetypeBrush:Bool = false;
	//Notetype格式刷
	var ntBrushText:FlxText;
	var copiedNoteType:String = "";
	override function create()
	{
		...
		var eventIcon:FlxSprite = new FlxSprite(195, 80).loadGraphic(Paths.image('eventArrow'));
		...
		leftIcon.setPosition(165, 50);
		rightIcon.setPosition(590, 50);
		...
		preRenderedSustains = new FlxTypedGroup<FlxSprite>();
		preRenderedNotes = new FlxTypedGroup<Note>();
		...
		bpmTxt = new FlxText(1030, 50, 0, "", 24);
		...
		ntBrushText= new FlxText(10,620,"",20);
		ntBrushText.scrollFactor.set();
		ntBrushText.font=Paths.font("vcr.ttf");
		ntBrushText.color = 0xFF00FF00;
		ntBrushText.borderStyle = FlxTextBorderStyle.OUTLINE;
		ntBrushText.borderColor = FlxColor.BLACK;
		add(ntBrushText);

		var BoolText= new FlxText(10,700,"Remake By Kevin7501",16);
		BoolText.scrollFactor.set();
		BoolText.font=Paths.font("vcr.ttf");
		BoolText.color = 0xFFFFFFFF;
		BoolText.borderStyle = FlxTextBorderStyle.OUTLINE;
		BoolText.borderColor = FlxColor.BLACK;
		add(BoolText);
		...
		add(preRenderedSustains);
		add(preRenderedNotes);
		...
		zoomTxt = new FlxText(10, 30, 0, "Zoom: 1 / 1", 24);
		zoomTxt.font = Paths.font("vcr.ttf");
	...
	override function update(elapsed:Float)
	{
		if(FlxG.keys.justPressed.F10)
			if(camYoffset == 240)camYoffset = 0;
			else camYoffset = 240;
		...
		camPos.y = strumLine.y + camYoffset;
			...
			if (FlxG.mouse.overlaps(curRenderedNotes))
			{
				curRenderedNotes.forEachAlive(function(note:Note)
				{
					if (FlxG.mouse.overlaps(note))
					{
						...
						{
							selectNote(note);
	//在noteType格式刷开启的状态下，ctrl + 单击箭头关闭格式刷
								if(ifNotetypeBrush){
									ifNotetypeBrush=false;
									ntBrushText.text = "";
							}
							
							if(note.noteData > -1)UI_box.selected_tab = 2;
							else UI_box.selected_tab = 1;
					//当我选中箭头时，直接切到Notes
						}
	...
	//在noteType格式刷开启的状态下，单击箭头并不删除，而是快速设置noteType
							if(ifNotetypeBrush && note.noteData > -1){
								selectNote(note);
		...
		if (!blockInput)
		{
			...
			if(curSelectedNote != null && curSelectedNote[1] > -1) {
				...
				if (FlxG.keys.justPressed.C){
					trace("NoteType Brush");
					ifNotetypeBrush = !ifNotetypeBrush;
					if(ifNotetypeBrush){
						ntBrushText.text = "NoteType Brush : " + curSelectedNote[3];
						copiedNoteType = curSelectedNote[3];
					}
					else ntBrushText.text = "";
	...
	*/
	...
	function reloadGridLayer() {
		...
		if(curSec > 0)
			{
				preGridBG = FlxGridOverlay.create(GRID_SIZE, GRID_SIZE, GRID_SIZE * 9, Std.int(GRID_SIZE * getSectionBeats(curSec - 1) * 4 * zoomList[curZoom]));
				leHeight = Std.int(gridBG.height + nextGridBG.height + preGridBG.height);
			}
			else preGridBG = new FlxSprite().makeGraphic(1, 1, FlxColor.TRANSPARENT);

		nextGridBG.y = gridBG.height;
		preGridBG.y = - preGridBG.height;
		
		gridLayer.add(preGridBG);
		...
		if(curSec > 0)
			{
				var gridBlack:FlxSprite = new FlxSprite(0, -preGridBG.height).makeGraphic(Std.int(GRID_SIZE * 9), Std.int(preGridBG.height), FlxColor.BLACK);
				gridBlack.alpha = 0.4;
				gridLayer.add(gridBlack);
			}

		var gridBlackLine:FlxSprite = new FlxSprite(gridBG.x + gridBG.width - (GRID_SIZE * 4), -preGridBG.height).makeGraphic(2, leHeight, FlxColor.BLACK);
		...
		var gridBlackLine:FlxSprite = new FlxSprite(gridBG.x + GRID_SIZE, -preGridBG.height).makeGraphic(2, leHeight, FlxColor.BLACK);
		gridLayer.add(gridBlackLine);
		updateGrid();
	...
	function updateGrid():Void
	{
		...
		preRenderedNotes.clear();
		preRenderedSustains.clear();
...
//PRE SECTION
//显示上一节谱面信息（主要是为了跨节长音的调试）
//必须用try，不然这个地方可能会crash
	try{
		if(curSec>0) {
				for (i in _song.notes[curSec-1].sectionNotes)
				{
					var note:Note = setupNoteData(i, true);
					note.alpha = 0.6;
					preRenderedNotes.add(note);
					if (note.sustainLength > 0)
					{
						preRenderedSustains.add(setupSusNote(note, beats));
					}
				}
			}
	}catch(e){
		//trace("crash if not try");
	}
